...
The optional @DETERMINISTIC@ keyword specifies that the aggregate is a deterministic function. This means that given a particular input, the aggregate will always produce the same output.
...
bc(sample). 
GRANT SELECT ON RESOURCE('data/keyspace1/table1') TO data_reader;

This form uses the internal string representation of the target resource, normalizing the variant syntax of the specific resource types.
The string representation is used by the authz subsystem internally, so this syntax enables DDL statements to be generated from extant permissions
e.g. from the contents of @system_auth.role_permissions@ if using internal authorization.
...
REVOKE EXECUTE ON RESOURCE('function/keyspace1/user_function[org.apache.cassandra.db.marshal.Int32Type]') FROM report_writer;
p. 
