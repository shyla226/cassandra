...
public class MultiNodeConcurrencyFactorTest extends TestBaseImpl
{
    ...
    {
        return cluster.get(1).callOnInstance(() -> Math.toIntExact(StorageProxy.rangeMetrics.roundTrips.getCount()));
    }

    private int getMaxRoundTrips()
    {
        return cluster.get(1).callOnInstance(() -> Math.toIntExact(StorageProxy.rangeMetrics.roundTrips.getSnapshot().getMax()));
    }
}
