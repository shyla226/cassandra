...
public class ThrottledUnfilteredIteratorTest extends CQLTester
{
    ...
    public void simpleThrottleTest(boolean skipOdd)
    {
        ...
        {
            origin = rows(metadata.partitionColumns(),
                          1,
    ...
    public void throttledPartitionIteratorTest()
    {
        ...
        for (int throttle = 2; throttle < 1200; throttle += 21)
        {
            origin = partitions(metadata.partitionColumns(),
                                new DeletionTime(0, 100),
                                createStaticRow(createCell(staticMetadata, 160)),
                                partitions);
            ...
            while (throttledIterator.hasNext())
            {
                ...
                UnfilteredRowIterator current = rows(metadata.partitionColumns(),
                                                     currentPartition,
                                                     new DeletionTime(0, 100),
                                                     createStaticRow(createCell(staticMetadata, 160)),
    ...
    private static UnfilteredRowIterator rows(PartitionColumns columns,
                                              int pk,
    ...
    private static UnfilteredPartitionIterator partitions(PartitionColumns columns,
                                                          DeletionTime partitionDeletion,
                                                          Row staticRow,
    ...
    private static Cell createCell(ColumnDefinition metadata, int v, long timestamp, int localDeletionTime)
    {
    ...
    public void testThrottledIteratorWithRangeDeletions() throws Exception
    {
        ...
        for (int batchSize = 2; batchSize <= 40; batchSize++)
        {
            ...
            try (UnfilteredPartitionIterator iterator = cmd.executeLocally(cmd.startOrderGroup()))
            {
            ...
            for (int i : live)
                assertTrue("Row " + i + " should be live",
                           partition.getRow(new Clustering(ByteBufferUtil.bytes((i))))
                                    .hasLiveData(nowInSec, cfs.metadata.enforceStrictLiveness()));
            for (int i : dead)
