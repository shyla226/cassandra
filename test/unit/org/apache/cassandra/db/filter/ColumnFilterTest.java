...
public class ColumnFilterTest
{
    ...
    public void testColumnFilterConstruction()
    {
        ...
        CFMetaData metadata = CFMetaData.Builder.create("ks", "table")
                                              ...
                                              .build();
        ColumnFilter columnFilter = ColumnFilter.all(metadata);
        assertTrue(columnFilter.includesAllColumns());
        assertEquals(metadata.partitionColumns(), columnFilter.fetchedColumns());
        assertNull(columnFilter.queriedColumns());
        ...
        assertFalse(columnFilter.includesAllColumns());
    ...
    public void testColumnFilterSelectionTypes()
    {
        ...
        assertFalse(filter.includesAllColumns());
        for (ColumnDefinition c : Iterables.concat(metadata.partitionColumns().regulars,
                                                 metadata.partitionColumns().statics))
        {
            if (c.equals(v1))
                                                 ...
                                                 metadata.partitionColumns().statics))
        {
            ...
            else if (c.equals(complex1))
            {
                ...
                assertTrue(tester.includes(b1));
                assertFalse(tester.includes(b2));
            }
            else
            {
                assertFalse(filter.includes(c));
    ...
    public void testColumnFilterWithColumnsAdded()
    {
        ...
        ColumnFilter filter = ColumnFilter.all(metadata);
        assertTrue(filter.includesAllColumns());
        assertTrue(filter.includes(v1));
        assertTrue(filter.includes(s1));
        assertTrue(filter.includes(complex1));
        assertTrue(filter.includes(v2)); // poor filter doesn't know any better
        assertTrue(filter.includes(s2));
        assertTrue(filter.includes(complex2));
        ...
        assertFalse(filter.includesAllColumns()); // the filter cannot any longer be sure that all columns are fetched
        assertTrue(filter.includes(v1));
        assertTrue(filter.includes(s1));
        assertTrue(filter.includes(complex1));
        assertFalse(filter.includes(v2)); // the filter now gets it that these new columns should be excluded
        assertFalse(filter.includes(s2));
        assertFalse(filter.includes(complex2));
    }
    ...
    public void testColumnFilterWithColumnsRemoved()
    {
        ...
        assertTrue(filter.includes(s1));
        assertTrue(filter.includes(complex1));
        assertTrue(filter.includes(v2)); // the filter should not fetch the columns that were removed
        assertTrue(filter.includes(s2));
        assertTrue(filter.includes(complex2));
    }
