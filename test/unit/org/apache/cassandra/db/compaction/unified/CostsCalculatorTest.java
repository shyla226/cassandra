...
public class CostsCalculatorTest
{
    ...
    public void setUp()
    {
        ...
        when(environment.makeExpMovAverage(any(TimeSource.class))).thenAnswer(ts -> new MovingAverageMock());
    }

    @Test
    public void testCreateAndClose()
    {
        CostsCalculator cost = new CostsCalculator(environment, timeSource, strategy, executorService, survivalFactor);
        assertNotNull(cost);
                           ...
                           double survivalFactor) throws InterruptedException
    {
        ...
        CostsCalculator cost = new CostsCalculator(environment, timeSource, strategy, executorService, survivalFactor, readMultiplier, writeMultiplier);
        assertNotNull(cost);
        assertNotNull(cost.toString());
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            timeSource.sleep(CostsCalculator.samplingPeriodMs, TimeUnit.MILLISECONDS);
            cost.sampleValues();
            assertNotNull(cost.toString());
    ...
    public void testNoBytesInserted()
    {
        ...
        CostsCalculator cost = new CostsCalculator(environment, timeSource, strategy, executorService, survivalFactor, 1, 1);
        assertNotNull(cost);
        assertNotNull(cost.toString());
