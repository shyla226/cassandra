...
public class WritetimeOrTTLTest extends CQLTester
{
    ...
    public void testNestedUDTs() throws Throwable
    {
        ...
        String type = createType(String.format("CREATE TYPE %%s (f1 frozen<%s>, f2 frozen<%<s>)", nestedType));
        createTable("CREATE TABLE %s (k int PRIMARY KEY, t " + type + ')');
    ...
    throws Throwable
    {
        ...
        {
            int actualTTL = row.getInt(ttlColumn);

            assertTTL(ttl, actualTTL);
        }
    }

    private void assertWritetimeAndTTL(String column, String where, List<Long> timestamp, List<Integer> ttls)
    throws Throwable
    {
        where = where == null ? "" : " WHERE " + where;

        // Verify write time
        assertRows(String.format("SELECT WRITETIME(%s) FROM %%s %s", column, where), row(timestamp));
    ...
    {
        String message = String.format("Element selection is only allowed on sets and maps, but %s is a list", list);
    ...
    {
        String message = String.format("Slice selection is only allowed on sets and maps, but %s is a list", list);
        assertInvalidThrowMessage(message,
                                  InvalidRequestException.class,
                                  String.format("SELECT WRITETIME(%s) FROM %%s", column));
        assertInvalidThrowMessage(message,
