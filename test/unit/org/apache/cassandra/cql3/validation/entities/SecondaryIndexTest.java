...
public class SecondaryIndexTest extends CQLTester
{
    ...
    public void testIndexOnCollectionsBackingTableKeyValidator() throws Throwable
    {
        ...
        assertInvalidMessage("Cannot create keys() index on frozen column frozen_list. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (KEYS(frozen_list))");
        assertInvalidMessage("Cannot create entries() index on frozen column frozen_list. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (ENTRIES(frozen_list))");
        assertInvalidMessage("Cannot create values() index on frozen column frozen_list. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (VALUES(frozen_list))");

        // Unsupported index types for frozen set
        assertInvalidMessage("Cannot create keys() index on frozen column frozen_set. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (KEYS(frozen_set))");
        assertInvalidMessage("Cannot create entries() index on frozen column frozen_set. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (ENTRIES(frozen_set))");
        assertInvalidMessage("Cannot create values() index on frozen column frozen_set. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (VALUES(frozen_set))");

        // Unsupported index types for frozen map
        assertInvalidMessage("Cannot create keys() index on frozen column frozen_map. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (KEYS(frozen_map))");
        assertInvalidMessage("Cannot create entries() index on frozen column frozen_map. " +
                             "Frozen collections only support full() indexes",
                             "CREATE INDEX ON %s (ENTRIES(frozen_map))");
        assertInvalidMessage("Cannot create values() index on frozen column frozen_map. " +
                             "Frozen collections only support full() indexes",
