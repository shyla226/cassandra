...
{
    public static List<Pair<ByteBuffer, IntArrayList>> buildStringTermsEnum(int terms, int postings, Supplier<String> termsGenerator, IntSupplier postingsGenerator)
    {
        final List<ByteBuffer> sortedTerms = Stream.generate(termsGenerator)
                .distinct()
                .limit(terms)
                .map(UTF8Type.instance::decompose)
                .sorted(UTF8Type.instance)
                .collect(toList());

        final List<Pair<ByteBuffer, IntArrayList>> termsEnum = new ArrayList<>();
