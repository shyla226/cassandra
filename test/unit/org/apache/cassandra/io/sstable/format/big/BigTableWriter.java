...
public class BigTableWriter extends SSTableWriter
{
    ...
    private void afterAppend(DecoratedKey decoratedKey, long dataEnd, BigRowIndexEntry index, ByteBuffer indexInfo) throws IOException
    {
     ...
     */
    public BigRowIndexEntry append(UnfilteredRowIterator iterator)
    {
        ...
        try (UnfilteredRowIterator collecting = Transformation.apply(iterator, new StatsCollector(metadataCollector)))
        {
            ...
            // afterAppend() writes the partition key before the first BigRowIndexEntry - so we have to add it's
            ...
            BigRowIndexEntry entry = BigRowIndexEntry.create(startPosition, indexFilePosition,
    ...
    private BigRowIndexEntry.IndexSerializer getRowIndexEntrySerializer()
    {
    ...
    class IndexWriter extends AbstractTransactional implements Transactional
    {
        ...
        IndexWriter(long keyCount)
        {
            ...
            builder = new FileHandle.Builder(descriptor.filenameFor(Component.PRIMARY_INDEX)).mmapped(DatabaseDescriptor.getIndexAccessMode() == Config.AccessMode.mmap);
        ...
        public void append(DecoratedKey key, BigRowIndexEntry indexEntry, long dataEnd, ByteBuffer indexInfo) throws IOException
        {
            bf.add(key);
        ...
        protected void doPrepare()
        {
            ...
            {
                saveSummary(descriptor, first, last, indexSummary);
            }
        }
