...
public class PerRangeReducingKeyIteratorTest
{
    ...
    public void testSSTablesAreRequired() throws IOException
    {
        ...
        Collection<AbstractBounds<Token>> bounds = ImmutableList.of(new Range<Token>(partitioner.getMinimumToken(), partitioner.getMaximumToken()));
        PerRangeReducingKeyIterator demux = new PerRangeReducingKeyIterator(ImmutableList.of(), bounds);
    }
    ...
    public void testBoundsAreRequired() throws IOException
    {
        ...
        PerRangeReducingKeyIterator demux = new PerRangeReducingKeyIterator(sstables, ImmutableList.of());
    }
    ...
    public void testHasNextIdempotency() throws IOException
    {
        ...
        Set<SSTableReader> sstables = store.getLiveSSTables();
        Collection<AbstractBounds<Token>> bounds = ImmutableList.of(new Range<Token>(partitioner.getMinimumToken(), partitioner.getMaximumToken()));
        PerRangeReducingKeyIterator demux = new PerRangeReducingKeyIterator(sstables, bounds);

        Assert.assertTrue(demux.hasNext());
        Assert.assertTrue(demux.hasNext());

        Assert.assertNotNull(demux.next());

        Assert.assertFalse(demux.hasNext());
        Assert.assertFalse(demux.hasNext());
    }
