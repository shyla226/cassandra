...
public class RateBasedBackPressureTest
{
    ...
    public void testBackPressurePastTimeout() throws Exception
    {
        ...
        // 2 seconds of response time, so the the timer is scheduled after 2 seconds:
        strategy.apply(replicaGroup, 4, TimeUnit.SECONDS);
    ...
    public static class TestableBackPressure extends RateBasedBackPressure
    {
        ...
        @Override
        public CompletableFuture<Boolean> doRateLimit(Callable<Boolean> rateLimiter,
                                                      BiConsumer<Runnable, Long> timer,
                                                      double limitedRate, long timeoutInNanos)
        {
            Callable<Boolean> fakeLimiter = () -> {
            ...
            BiConsumer<Runnable, Long> fakeTimer = (r, t) -> {
                timeout = t;
            };

            return super.doRateLimit(fakeLimiter, fakeTimer, limitedRate, timeoutInNanos)
                .thenApply(v -> {
                    acquired = v;
                    applied = true;
                    if (acquired)
