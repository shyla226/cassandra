...
public class LegacyAuthFailTest extends CQLTester
{
    ...
    public void testLegacyTablesCheck() throws Throwable
    {
        ...
        testCheckFailure(new ArrayList<>(StartupChecks.LEGACY_AUTH_TABLES), tester);
    }
    ...
    public void testObsoleteTablesCheck() throws Throwable
    {
        ...
        testCheckFailure(new ArrayList<>(StartupChecks.OBSOLETE_AUTH_TABLES), tester);
    }
    ...
    {
        execute(format("DROP TABLE %s.%s", AuthKeyspace.NAME, table));
    }

    private void createLegacyTable(String table) throws Throwable
    {
        execute(format("CREATE TABLE %s.%s (id int PRIMARY KEY, val text)", AuthKeyspace.NAME, table));
    }

    private void createKeyspace() throws Throwable
    {
        execute(format("CREATE KEYSPACE IF NOT EXISTS %s WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}", AuthKeyspace.NAME));
    }
