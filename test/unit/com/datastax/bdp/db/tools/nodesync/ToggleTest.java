...
public class ToggleTest extends CQLTester
{
    ...
    public void testExecute() throws Throwable
    {
        ...
        String t1 = "t1";
        String t2 = "\"t2\"";
        String t3 = "\"T3\""; // upper case
        String t4 = "\"schema\""; // quoted reserved keyword
        ...
        execute(String.format("CREATE TABLE %s.%s (k int, c int, PRIMARY KEY (k, c))", k1, t3));
        execute(String.format("CREATE TABLE %s.%s (k int, c int, PRIMARY KEY (k, c))", k1, t4));
        ...
        execute(String.format("CREATE TABLE %s.%s (k int, c int, PRIMARY KEY (k, c))", k2, t3));
        execute(String.format("CREATE TABLE %s.%s (k int, c int, PRIMARY KEY (k, c))", k2, t4));
        ...
        String k2t3 = k2 + '.' + t3;
        String k2t4 = k2 + '.' + t4;
        Set<String> userTables = newHashSet(k1t1, k1t2, k1t3, k1t4, k2t1, k2t2, k2t3, k2t4);
        ...
        assertValidExecution(session, null, newArrayList(k1t3), newHashSet(k1t3));
        assertValidExecution(session, null, newArrayList(k1t2, k1t3, k1t4, k2t1, k2t2, k2t4), newHashSet(k1t2, k1t3, k1t4, k2t1, k2t2, k2t4));
        ...
        assertValidExecution(session, k1, newArrayList("*"), newHashSet(k1t1, k1t2, k1t3, k1t4));
        ...
        assertInvalidExecution(session, null, newArrayList(k1 + ".T3"), NodeSyncException.class,
                               String.format("Table [%s.T3] does not exist.", k1));
        assertInvalidExecution(session, null, newArrayList(k1 + ".\"t3\""), NodeSyncException.class,
                               String.format("Table [%s.\"t3\"] does not exist.", k1));
                                             ...
                                             boolean enable)
    {
        ...
        expectedTableNames.stream()
                          .map(s -> DOUBLE_QUOTE_PATTERN.matcher(s).replaceAll(""))
                          .forEach(t -> expectedStatus.put(t, enable));
