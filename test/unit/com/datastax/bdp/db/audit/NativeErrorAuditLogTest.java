...
public class NativeErrorAuditLogTest extends CQLTester
{
    ...
    {
        return ClientState.forExternalCalls(new InetSocketAddress(9042), null)
                          .login(AuthenticatedUser.ANONYMOUS_USER)
                          .blockingGet();
    }

    private static QueryState getQueryState() throws AuthenticationException
    {
        return new QueryState(getClientState(), UserRolesAndPermissions.ANONYMOUS);
    }
    ...
    {
        QueryHandler queryHandler = QueryProcessor.instance;
        MD5Digest preparedId = TPCUtils.blockingGet(queryHandler.prepare(select, getQueryState(), null)).statementId;
    ...
    public void checkProcessBatch() throws Exception
    {
        ...
        ModificationStatement stmt = (ModificationStatement) QueryProcessor.getStatement(insert, getQueryState()).statement;
    ...
    public void checkPrepare() throws Exception
    {
        ...
        try
        {
            TPCUtils.blockingGet(queryHandler.prepare("SELECT * FROM nothing.nothing;", getQueryState(), null));
            Assert.fail("Expecting RequestExecutionException or RequestValidationException");
        }
