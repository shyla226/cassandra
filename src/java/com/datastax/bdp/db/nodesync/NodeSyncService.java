...
public class NodeSyncService implements NodeSyncServiceMBean
{
    ...
    private volatile NodeSyncState state;
    ...
    public synchronized CompletableFuture<Boolean> enableAsync()
    {
        ...
        state = new NodeSyncState(this);
        scheduler = new ValidationScheduler(state);
        ...
        // miss a table creation/update due to a race. This might make us create more than one ContinuousValidationProposer
    ...
    public synchronized CompletableFuture<Boolean> disableAsync(boolean force)
    {
        ...
        return disableInternal(force, true).thenApply(x -> true);
    }
    ...
    public boolean canPurge(TableMetadata table, int gcBefore)
    {
        ...
        ValidationScheduler scheduler = this.scheduler; // protect against concurrent shutdowns
        if (scheduler == null) // NodeSync isn't running
            return true;

        ContinuousValidationProposer proposer = scheduler.getContinuousProposer(table);
    ...
    public void startUserValidation(Map<String, String> optionMap)
    {
        ...
        scheduler.addUserValidation(UserValidationOptions.fromMap(optionMap));
    }
    ...
    public void cancelUserValidation(String id)
    {
        ...
        UserValidationProposer proposer = scheduler.getUserValidation(id);
        if (proposer == null)
    ...
    private static class LogReporter implements Runnable
    {
        ...
        private LogReporter(NodeSyncMetrics metrics, ValidationScheduler scheduler)
        {
            ...
            this.lastProposerCount = scheduler.continuouslyValidatedTables();
            this.lastScheduledValidations = scheduler.scheduledValidations();
        }

        public void run()
        {
            int currentProposerCount = scheduler.continuouslyValidatedTables();
            ...
            if (currentScheduledValidations == lastScheduledValidations && lastProposerCount == 0 && currentProposerCount == 0)
                return;
