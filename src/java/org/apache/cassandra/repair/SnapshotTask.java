...
public class SnapshotTask extends AbstractFuture<InetAddress> implements RunnableFuture<InetAddress>
{
    ...
    public void run()
    {
        MessagingService.instance()
                        .sendSingleTarget(Verbs.REPAIR.SNAPSHOT.newRequest(endpoint, new SnapshotMessage(desc)))
                        .thenAccept(r -> set(endpoint))
                        .exceptionally(f -> { setException(new RuntimeException("Could not create snapshot at " + endpoint)); return null; });
    }
}