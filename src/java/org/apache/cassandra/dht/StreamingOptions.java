...
public final class StreamingOptions
{
        ...
        if (includedDCs != null && excludedDCs != null)
        {
            for (String dc : includedDCs.keySet())
            {
                if (excludedDCs.containsKey(dc) && excludedDCs.get(dc).isEmpty() == includedDCs.get(dc).isEmpty())
                    throw new IllegalArgumentException("The " + BOOTSTRAP_INCLUDE_DCS + " and " + BOOTSTRAP_EXCLUDE_DCS + " system properties are conflicting for the datacenter: " + dc);
            }
        }
    ...
    /**
     * Ensures that arguments specified do not conflict with each others.
     *
     * @param tokenMetadata to validate sources (endpoints), DCs and racks
                                          ...
                                          Map<String, Racks> excludedDCs)
    {
        if (includedSources != null && (excludedSources != null ||  includedDCs != null || excludedDCs != null))
            throw new IllegalArgumentException("The " + ARG_INCLUDE_SOURCES + " argument cannot be used together with"
                                               + " the " + ARG_EXCLUDE_SOURCES + ", " + ARG_INCLUDE_DC_NAMES + " or " + ARG_EXCLUDE_DC_NAMES + " arguments");

        if (excludedSources != null && (includedDCs != null || excludedDCs != null))
            throw new IllegalArgumentException("The " + ARG_EXCLUDE_SOURCES + " argument cannot be used together with"
                                               + " the " + ARG_INCLUDE_SOURCES + ", " + ARG_INCLUDE_DC_NAMES + " or " + ARG_EXCLUDE_DC_NAMES + " arguments");
        ...
        if (includedDCs != null & excludedDCs != null)
        {
            ...
            {
                if (excludedDCs.containsKey(dc) && excludedDCs.get(dc).isEmpty() == includedDCs.get(dc).isEmpty())
                    throw new IllegalArgumentException("The " + ARG_INCLUDE_DC_NAMES + " and " + ARG_EXCLUDE_DC_NAMES + " arguments are conflicting for the datacenter: " + dc);
            }
        }
    ...
    private static Set<String> getKeyspacesFromProperty(String property)
    {
        ...
        for (String name : list)
        {
            if (!nonLocalStrategyKeyspaces.contains(name))
                throw new IllegalArgumentException(String.format("The %s keyspace specified within the %s system property is not an existing non local strategy keyspace", name, property));

            if (!keyspaces.add(name))
                throw new IllegalArgumentException(String.format("The %s keyspace specified within the %s system property must be specified only once", name, property));
        }
                                                         ...
                                                         List<String> argument)
    {
        ...
        for (String hostName : argument)
        {
            try
            {
                ...
                {
                    throw new IllegalArgumentException(String.format("The %s source must be specified only once in the %s %s", hostName, argumentName, argumentType));
                }
            }
            catch (UnknownHostException ex)
            {
                throw new IllegalArgumentException(String.format("The %s source specified within the %s %s is unknown", hostName, argumentName, argumentType));
            }
        }
                                                                 ...
                                                                 List<String> argument)
    {
        ...
        for (String name : argument)
        {
            ...
            if (sep == -1)
            {
                ...
                {
                    if (racks.isEmpty())
                        throw new IllegalArgumentException(String.format("The %s datacenter must be specified only once in the %s %s", name, argumentName, argumentType));

                    throw new IllegalArgumentException(String.format("The %s %s contains both a rack restriction and a datacenter restriction for the %s datacenter", argumentName, argumentType, name));
                }
            ...
            else
            {
                ...
                {
                    if (racks.isEmpty())
                        throw new IllegalArgumentException(String.format("The %s %s contains both a rack restriction and a datacenter restriction for the %s datacenter", argumentName, argumentType, dcName));

                    if (!racks.addRack(rackName))
                        throw new IllegalArgumentException(String.format("The %s rack must be specified only once in the %s %s", name, argumentName, argumentType));
                }
            }
        }
