...
public class CostsCalculator
{
    ...
    private final MovingAverageDelta partitionsRead;
                    ...
                    double writeMultiplier)
    {
        ...
        this.partitionsRead = new MovingAverageDelta(env.makeExpMovAverage(timeSource));
        this.bytesInserted = new MovingAverageDelta(env.makeExpMovAverage(timeSource));
        this.numSSTables = env.makeExpMovAverage(timeSource);
        this.spaceUsed = env.makeExpMovAverage(timeSource);
    ...
    void sampleValues()
    {
        ...
        try
        {
            partitionsRead.update(env.partitionsRead());
            bytesInserted.update(env.bytesInserted());
    ...
    public double getReadCostForQueries(int RA)
    {
        ...
        try
        {
            return getReadCost(partitionsRead.avg.get()) * Math.min(1 + env.bloomFilterFpRatio() * RA / survivalFactor, RA) * readMultiplier;
        }
        finally
        {
            readLock.unlock();
    ...
    public double getWriteCostForQueries(int WA)
    {
        ...
        try
        {
            double bytesInserted = this.bytesInserted.avg.get();
    ...
    public MovingAverage partitionsRead()
    {
        return partitionsRead.avg;
    }

    public MovingAverage numSSTables()
    {
        return numSSTables;
    }

    public MovingAverage spaceUsed()
    {
        return spaceUsed;
    }
    ...
    @NotThreadSafe
    private static final class MovingAverageDelta
    {
        private final MovingAverage avg;
        private volatile double prev;

        MovingAverageDelta(MovingAverage avg)
        {
            this.avg = avg;
