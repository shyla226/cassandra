...
public class ShardedMultiWriter implements SSTableMultiWriter
{
                              ...
                              List<PartitionPosition> boundaries)
    {
        ...
        this.estimatedSSTables = (int) Math.max(1, Math.ceil(cfs.getFlushSize() / minSstableSizeInBytes));

        this.currentBoundary = 0;
        this.currentWriter = 0;
        this.writers[currentWriter] = createWriter();
    }

    private SSTableMultiWriter createWriter()
    {
        Descriptor newDesc = cfs.newSSTableDescriptor(descriptor.directory);
        return SimpleSSTableMultiWriter.create(newDesc,
                                               keyCount / estimatedSSTables,
                                               repairedAt,
                                               pendingRepair,
                                               isTransient,
                                               cfs.metadata,
    ...
    public boolean append(UnfilteredRowIterator partition)
    {
        ...
        if (boundaryCrossed && writers[currentWriter].getFilePointer() >= minSstableSizeInBytes)
        {
