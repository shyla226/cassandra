...
class ViewBuilder
{
    ...
    public void start()
    {
        if (TPCUtils.blockingGet(SystemKeyspace.isViewBuilt(ksName, view.name)))
        {
            logger.debug("View already marked built for {}.{}", ksName, view.name);
            if (!TPCUtils.blockingGet(SystemKeyspace.isViewStatusReplicated(ksName, view.name)))
                updateDistributed();
        }
        else
        {
    ...
    private void loadStatus()
    {
        builtRanges.clear();
        pendingRanges.clear();
        TPCUtils.blockingGet(SystemKeyspace.getViewBuildStatus(ksName, view.name))
                .forEach((range, pair) ->
                         {
