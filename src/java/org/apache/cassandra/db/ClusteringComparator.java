...
public class ClusteringComparator implements Comparator<Clusterable>
{
     ...
     *    BOTTOM                    -> 20
     */
    private class ByteComparableClustering implements ByteComparable
    {
        private final ClusteringPrefix src;

        ByteComparableClustering(ClusteringPrefix src)
        {
            this.src = src;
        }
        ...
        public ByteSource asComparableBytes(Version version)
        {
            return new ByteSource()
            {
                ...
                public int next()
                {
                    ...
                    current = subtype(srcnum).asComparableBytes(src.get(srcnum), version);
                    if (current == null)
