...
public class QueryProcessor implements QueryHandler
{
    ...
    throws RequestExecutionException, RequestValidationException
    {
        ...
        Single<ResultMessage> ret = Single.defer(() -> {
            try
            {
                return processStatementInternal(statement, queryState, options, queryStartNanoTime);
            }
            catch (TPCUtils.WouldBlockException ex)
            {
                ...
                Single<ResultMessage> single = Single.defer(() ->
                                                            processStatementInternal(statement,
                                                                                     queryState,
                                                                                     options,
                                                                                     queryStartNanoTime));
    ...
    private SingleSource<? extends ResultMessage> processStatementInternal(CQLStatement statement, QueryState queryState,
                                                                           QueryOptions options, long queryStartNanoTime)
    {
        CQLStatement filtering = SystemKeyspacesFilteringRestrictions.maybeAdd(statement, queryState);

        if (queryState.hasUser())
            filtering.checkAccess(queryState);
        filtering.validate(queryState);
        return filtering.execute(queryState, options, queryStartNanoTime);
    }
                                                                         ...
                                                                         boolean storeStatementOnDisk)
    {
        ...
        return observable.map(resultSet -> result);
    }
    ...
    public Single<ResultMessage> processBatch(BatchStatement batch, QueryState queryState, BatchQueryOptions options, long queryStartNanoTime)
    {
        ...
        return Single.defer(() -> {
            ...
            {
                return RxThreads.subscribeOnIo(Single.defer(() -> processBatchInternal(batch, options, queryStartNanoTime, state)),
    ...
    throws RequestValidationException
    {
        ...
        if (statement instanceof CFStatement)
        {
            ...
            Optional<String> ks = cfStatement.keyspaceOptional();
            if (ks.isPresent() && Schema.instance.getKeyspaceMetadata(ks.get()) != null)
            {
                // Check for DESCRIBE permission on keyspace, if system-ks-filtering is enabled
                SystemKeyspacesFilteringRestrictions.hasMandatoryPermissions(ks.get(), state);
            }
        }
