...
abstract class ElementsSelector extends Selector
{
     ...
     */
    private static ByteBuffer EMPTY_FROZEN_COLLECTION = ByteBufferUtil.bytes(0);
    ...
    {
        return type.getSerializer().getIndexFromSerialized(output, key, keyType(type));
    }
    ...
    static final class ElementSelector extends ElementsSelector
    {
        ...
        private Timestamps getElementTimestamps(ProtocolVersion protocolVersion,
        ...
        {
            Serializer serializer = serializers.get(version);
            return serializer.serializedSize(selected) + ByteBufferUtil.serializedSizeWithVIntLength(key);
        }
        ...
        {
            Serializer serializer = serializers.get(version);
            serializer.serialize(selected, out);
    ...
    static final class SliceSelector extends ElementsSelector
    {
        ...
        {
            ByteBuffer output = selected.getOutput(protocolVersion);
            if (output == null || isCollectionEmpty(output))
                return Timestamps.NO_TIMESTAMP;

            return timestamps.slice(getIndexRange(output, from, to) );
        }
        ...
        {
            Serializer serializer = serializers.get(version);
            return serializer.serializedSize(selected)
        ...
        protected void serialize(DataOutputPlus out, ReadVersion version) throws IOException
        {
            Serializer serializer = serializers.get(version);
            serializer.serialize(selected, out);
            ByteBufferUtil.writeWithVIntLength(from, out);
            ByteBufferUtil.writeWithVIntLength(to, out);
        }
    }
}
