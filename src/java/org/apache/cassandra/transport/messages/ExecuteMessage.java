...
public class ExecuteMessage extends Message.Request
{
    ...
    {
        return ClientState.getCQLQueryHandler().getPrepared(statementId).statement.supportsPipelineExecution();
    }
    ...
    public void executePipeline(RequestContext requestContext)
    {
        try
        {
            ...
            options.prepare(prepared.boundNames);
            requestContext.statement = prepared.statement;
            ...
            {
                tracingId = UUIDGen.getTimeUUID();
                requestContext.queryState.prepareTracingSession(tracingId);
            }

            if (requestContext.queryState.traceNextQuery())
            {
                requestContext.queryState.createTracingSession(getCustomPayload());
                ...
                Tracing.instance.begin("Execute CQL3 prepared query", requestContext.queryState.getClientAddress(), builder.build());
            }

            // Some custom QueryHandlers are interested by the bound names. We provide them this information
            // by wrapping the QueryOptions.
            requestContext.queryOptions = QueryOptions.addColumnSpecifications(options, prepared.boundNames);
            // Single<? extends ResultMessage> obs = handler.processPrepared(statement, state, queryOptions, getCustomPayload(), queryStartNanoTime);
            final UUID finalTracingId = tracingId;
            requestContext.messagePostProcessor = (response -> {
