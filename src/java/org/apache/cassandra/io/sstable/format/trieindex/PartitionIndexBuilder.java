...
import static org.apache.cassandra.io.sstable.format.trieindex.PartitionIndex.source;
...
public class PartitionIndexBuilder implements AutoCloseable
{
    ...
    */
    public PartitionIndexBuilder addEntry(DecoratedKey decoratedKey, long position) throws IOException
    {
        ...
        {
            ByteSource current = source(decoratedKey);
            ByteSource prev = source(lastKey);
            int diffPoint = ByteSource.diffPoint(prev, current);
            ByteSource prevPrefix = ByteSource.cut(prev, Math.max(diffPoint, lastDiffPoint));
    ...
    public long complete() throws IOException
    {
        ...
        {
            ByteSource prev = source(lastKey);
            ByteSource prevPrefix = ByteSource.cut(prev, lastDiffPoint);
            trieWriter.add(prevPrefix, lastPayload);
        }
