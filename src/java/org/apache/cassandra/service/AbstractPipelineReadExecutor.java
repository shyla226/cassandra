 ...
 */
public abstract class AbstractPipelineReadExecutor
{
    private static final Logger logger = LoggerFactory.getLogger(AbstractPipelineReadExecutor.class);

    protected final ReadCommand command;
    protected final List<InetAddress> targetReplicas;
    protected final PipelineReadCallback handler;
    protected final TraceState traceState;

    AbstractPipelineReadExecutor(Keyspace keyspace, ReadCommand command, List<InetAddress> targetReplicas, RequestContext requestContext)
    {
        this.command = command;
        this.targetReplicas = targetReplicas;
        this.handler = new PipelineReadCallback(new DigestResolver(keyspace, command, requestContext.queryOptions.getConsistency(), targetReplicas.size()), command, targetReplicas, requestContext);
    ...
    private void makeRequests(ReadCommand readCommand, Iterable<InetAddress> endpoints)
    {
        ...
        if (hasLocalEndpoint)
        {
            ...
            {
                NettyRxScheduler.instance().scheduleDirect(() -> new StorageProxy.PipelineLocalReadRunnable(command, handler).runMayThrow());
            }
        }
    }
    ...
    */
     ...
     */
    public static AbstractPipelineReadExecutor getReadExecutor(SinglePartitionReadCommand command, RequestContext requestContext) throws UnavailableException
    {
        ...
        if (retry.equals(SpeculativeRetryParam.NONE)
            ...
            return new NeverSpeculatingReadExecutor(keyspace, command, targetReplicas, requestContext);
        ...
        if (targetReplicas.size() == allReplicas.size())
        {
            ...
            return new AlwaysSpeculatingReadExecutor(keyspace, cfs, command, targetReplicas, requestContext);
        }
        ...
        if (retry.equals(SpeculativeRetryParam.ALWAYS))
            ...
            return new SpeculatingReadExecutor(keyspace, cfs, command, targetReplicas, requestContext);
    }

    public static class NeverSpeculatingReadExecutor extends AbstractPipelineReadExecutor
    {
        public NeverSpeculatingReadExecutor(Keyspace keyspace, ReadCommand command, List<InetAddress> targetReplicas, RequestContext requestContext)
        {
            super(keyspace, command, targetReplicas, requestContext);
        }
    ...
    private static class SpeculatingReadExecutor extends AbstractPipelineReadExecutor
    {
    ...
    private static class AlwaysSpeculatingReadExecutor extends AbstractPipelineReadExecutor
    {
        private final ColumnFamilyStore cfs;
