...
import org.apache.cassandra.index.sai.metrics.QueryEventListener.PostingListEventListener;
...
public class PostingsReader implements OrdinalPostingList
{
    ...
    private final LongArray blockMaxValues; // ints only
    private final SeekingRandomAccessInput seekingInput;
    private final PostingListEventListener listener;
    ...
    @VisibleForTesting
    PostingsReader(IndexInput input, long summaryOffset, PostingListEventListener listener) throws IOException
    {
        this(input, new BlocksSummary(input, summaryOffset, () -> {}), listener);
    }

    PostingsReader(IndexInput input, BlocksSummary summary, PostingListEventListener listener) throws IOException
    {
        this.input = input;
    ...
    static class BlocksSummary
    {
        ...
        BlocksSummary(IndexInput input, long offset) throws IOException
        {
    ...
    private int binarySearchBlock(int targetRowID)
    {
        ...
        while (low <= high)
        {
            ...
            int midVal = Math.toIntExact(blockMaxValues.get(mid));
    ...
    private void reBuffer() throws IOException
    {
        ...
        final int left = numPostings - totalPostingsRead;
        assert left > 0;
